# -*- coding: utf-8 -*-
"""
Created on Thu Jul  9 16:43:17 2020
...
X = np.concatenate((array_rain_hist_1,array_rain_CDS))
y = np.concatenate((array_flood_hist_1,array_flood_CDS))
X = X.reshape((129,78,1))
y = y.reshape((129,361315,1))

X_val = array_rain_hist_2
X_val = X_val.reshape((28,78,1))

y_val = array_flood_hist_2
y_val = y_val.reshape((28,361315,1))
...
# event 4
X_pred_1 = X[4]
X_pred_1 = np.reshape(X_pred_1,(1,78,1))
y_sim_1 = y[4]
y_sim_1 = np.reshape(y_sim_1,(1,361315,1))
# event 11
X_pred_2 = X[11]
X_pred_2 = np.reshape(X_pred_2,(1,78,1))
y_sim_2 = y[11]
y_sim_2 = np.reshape(y_sim_2,(1,361315,1))
# event 16
X_pred_3 = X[16]
X_pred_3 = np.reshape(X_pred_3,(1,78,1))
y_sim_3 = y[16]
y_sim_3 = np.reshape(y_sim_3,(1,361315,1))
...
del X_pred_1, X_pred_2, X_pred_3, y_sim_1, y_sim_2
del y_sim_3, array_flood_CDS, array_flood_hist_1, array_rain_CDS, array_rain_hist_1
#%%
...
model = Sequential()
model.add(LSTM(78,activation='relu',kernel_initializer='he_normal',input_shape=(78,1),return_sequences=True))
# input_shape=(timestep,features)
#model.add(Dense(78, activation='tanh', kernel_initializer='he_normal'))
model.add(LSTM(78,activation='relu', kernel_initializer='he_normal'))
...
model.summary()
# early stopping:
monitor = EarlyStopping(monitor='val_loss', min_delta=1e-7, patience=70,verbose=1, mode='auto',restore_best_weights=True)
# fit the model:
history = model.fit(X, y,epochs=200, batch_size=32, verbose=1,validation_data = (X_val,y_val), callbacks=[monitor])

epochs = monitor.stopped_epoch

#model.save(os.path.join(r'C:\Users\boehm\Google_Drive\Masterarbeit\Part4_ANN_versions\ANNs\Models\07_RNN','model_epochs31_val_add_hist'))
#%% load model
model = tf.keras.models.load_model(os.path.join(r'C:\Users\boehm\Google_Drive\Masterarbeit\Part4_ANN_versions\ANNs\Models\07_RNN','model_epochs31_val_add_hist'))
#%% visualize training
plt.figure()
...
plt.ylabel('loss')
plt.xlabel('epoch')
plt.legend(['train', 'val - additional hist ev'], loc='upper left')
plt.show()
#%% prediction 
X_pred1 = X_test[0].reshape(1,78,1)
y_sim1 = y_test[0].reshape(1,361315)

X_pred2 = X_test[1].reshape(1,78,1)
y_sim2 = y_test[1].reshape(1,361315)

X_pred3 = X_test[2].reshape(1,78,1)
...
# np.save(os.path.join(link_folder,"07_ep31_ev1.npy"),yhat1)
# np.save(os.path.join(link_folder,"07_ep31_ev2.npy"),yhat2)
# np.save(os.path.join(link_folder,"07_ep31_ev3.npy"),yhat3)
#%% for difference maps
...
np.save(os.path.join(link_folder,"07_ep31_difference_ev1.npy"),difference_1)
np.save(os.path.join(link_folder,"07_ep31_difference_ev2.npy"),difference_2)
np.save(os.path.join(link_folder,"07_ep31_difference_ev3.npy"),difference_3)
#%% confusion matrix
...
np.save(os.path.join(link_folder,"07_ep31_hist_miss_ev1.npy"),y_diff_1_map)
np.save(os.path.join(link_folder,"07_ep31_hist_miss_ev2.npy"),y_diff_2_map)
np.save(os.path.join(link_folder,"07_ep31_hist_miss_ev3.npy"),y_diff_3_map)