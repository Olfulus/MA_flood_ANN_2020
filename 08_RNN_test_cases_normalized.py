...
# fit the model:
history = model.fit(X, y,epochs=30, batch_size=32, verbose=1,validation_data =(X_test,y_test))#, callbacks=[monitor])

#epochs = monitor.stopped_epoch

#model.save(os.path.join(r'C:\Users\boehm\Google_Drive\Masterarbeit\Part4_ANN_versions\ANNs\Models\08_RNN','model_epochs20_dp02'))

#model = tf.keras.models.load_model(os.path.join(r'C:\Users\boehm\Google_Drive\Masterarbeit\Part4_ANN_versions\ANNs\Models\06_RNN','model_epochs15_es_45_pat_30'))
#%% visualize training
plt.figure()
...
yhat2 = np.exp(yhat2)
yhat3 = np.exp(yhat3)
y_sim1 = np.exp(y_sim1)
y_sim2 = np.exp(y_sim2)
y_sim3 = np.exp(y_sim3)
