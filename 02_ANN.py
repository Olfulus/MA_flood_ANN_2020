...
model.add(Dense(78, activation='relu', kernel_initializer='he_normal',
                input_shape=(n_features,))) #hidden 1
model.add(Dropout(0.2))
model.add(Dense(78, activation='tanh', kernel_initializer='he_normal')) #hidden 2
model.add(Dense(361315)) # output
...
# fit the model:
history = model.fit(X, y,epochs=80, batch_size=32, verbose=1, validation_data = (X_val,y_val))#; callbacks=[monitor]
...
# save model
#model.save(os.path.join(r'C:\Users\boehm\Google_Drive\Masterarbeit\Part4_ANN_versions\ANNs\Models\02_FFNN','model_epochs80_dp02'))
...
#%% load model
model = tf.keras.models.load_model(os.path.join(os.path.join(r'C:\Users\boehm\Google_Drive\Masterarbeit\Part4_ANN_versions\ANNs\Models\02_FFNN','model_epochs80_dp02')))
...
# np.save(os.path.join(link_folder,"02_ev1.npy"),yhat1)
# np.save(os.path.join(link_folder,"02_ev2.npy"),yhat2)
# np.save(os.path.join(link_folder,"02_ev3.npy"),yhat3)
#%% confusion matrix
...
sns.heatmap(y_diff_1_map, ax = ax1, cmap=cmap,yticklabels=False,xticklabels=False)
sns.heatmap(y_diff_2_map, ax = ax2, cmap=cmap,yticklabels=False,xticklabels=False)
sns.heatmap(y_diff_3_map, ax = ax3, cmap=cmap,yticklabels=False,xticklabels=False)
fig.tight_layout()
plt.show()
...
np.save(os.path.join(link_folder,"02_ep80_difference_ev1.npy"),difference_1)
np.save(os.path.join(link_folder,"02_ep80_difference_ev2.npy"),difference_2)
np.save(os.path.join(link_folder,"02_ep80_difference_ev3.npy"),difference_3)

#%% save hit-miss
# link_folder = r'C:\Users\boehm\Google_Drive\Masterarbeit\Part4_ANN_versions\ANNs\hit-miss'
# np.save(os.path.join(link_folder,"02_ep80_hit_miss_ev1.npy"),y_diff_1_map)
# np.save(os.path.join(link_folder,"02_ep80_hit_miss_ev2.npy"),y_diff_2_map)
# np.save(os.path.join(link_folder,"02_ep80_hit_miss_ev3.npy"),y_diff_3_map)
...
fig.savefig(r'C:\Users\boehm\Google_Drive\Masterarbeit\Part4_ANN_versions\ANNs\Models\02_FFNN\hist_event3',dpi=300)
plt.show()
