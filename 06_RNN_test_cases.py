...
X = X.reshape((105,78,1))
y = y.reshape((105,361315,1))

X_val = X_val.reshape((24,78,1))
y_val = y_val.reshape((24,361315,1))
...
model.summary()
# early stopping:
monitor = EarlyStopping(monitor='val_loss', min_delta=1e-7, patience=100,verbose=1, mode='auto',restore_best_weights=True)
# fit the model:
history = model.fit(X, y,epochs=300, batch_size=32, verbose=1,validation_data = (X_val,y_val), callbacks=[monitor])

epochs = monitor.stopped_epoch

#model.save(os.path.join(r'C:\Users\boehm\Google_Drive\Masterarbeit\Part4_ANN_versions\ANNs\Models\06_RNN','model_epochs25_es_55_pat_30'))
#%% load model
model = tf.keras.models.load_model(os.path.join(r'C:\Users\boehm\Google_Drive\Masterarbeit\Part4_ANN_versions\ANNs\Models\06_RNN','model_epochs25_es_55_pat_30'))
#%% visualize training
plt.figure()
...
# np.save(os.path.join(link_folder,"06_ep25_ev1.npy"),yhat1)
# np.save(os.path.join(link_folder,"06_ep25_ev2.npy"),yhat2)
# np.save(os.path.join(link_folder,"06_ep25_ev3.npy"),yhat3)
#%% for difference maps
...
np.save(os.path.join(link_folder,"06_ep25_difference_ev1.npy"),difference_1)
np.save(os.path.join(link_folder,"06_ep25_difference_ev2.npy"),difference_2)
np.save(os.path.join(link_folder,"06_ep25_difference_ev3.npy"),difference_3)
#%% confusion matrix
...
np.save(os.path.join(link_folder,"06_ep25_hist_miss_ev1.npy"),y_diff_1_map)
np.save(os.path.join(link_folder,"06_ep25_hist_miss_ev2.npy"),y_diff_2_map)
np.save(os.path.join(link_folder,"06_ep25_hist_miss_ev3.npy"),y_diff_3_map)
