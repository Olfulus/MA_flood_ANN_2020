...
X = X.reshape((157,78,1))
y = y.reshape((157,361315,1))
...
del X_pred_1, X_pred_2, X_pred_3, y_sim_1, y_sim_2, y_sim_3, array_flood_CDS, array_flood_hist_1, array_rain_CDS, array_rain_hist_1, array_flood_hist_2, array_rain_hist_2 
#%%
...
model = Sequential()
model.add(LSTM(78,input_shape=(78,1),return_sequences=True))#,activation='relu',kernel_initializer='he_normal'
...
model.summary()
# early stopping:
#monitor = EarlyStopping(monitor='val_loss', min_delta=1e-7, patience=50,verbose=1, mode='auto',restore_best_weights=True)
# fit the model:
history = model.fit(X, y,epochs=80, batch_size=32, verbose=1,validation_data =(X_test,y_test))#, callbacks=[monitor])

#epochs = monitor.stopped_epoch

#model.save(os.path.join(r'C:\Users\boehm\Google_Drive\Masterarbeit\Part4_ANN_versions\ANNs\Models\08_RNN','model_epochs80_dp02_2'))
#%% load model
model = tf.keras.models.load_model(os.path.join(r'C:\Users\boehm\Google_Drive\Masterarbeit\Part4_ANN_versions\ANNs\Models\08_RNN','model_epochs80_dp02'))
#%% visualize training
plt.figure()
...
# np.save(os.path.join(link_folder,"08_ep80_ev1.npy"),yhat1)
# np.save(os.path.join(link_folder,"08_ep80_ev2.npy"),yhat2)
# np.save(os.path.join(link_folder,"08_ep80_ev3.npy"),yhat3)
#%% for difference maps
...
np.save(os.path.join(link_folder,"08_ep80_difference_ev1.npy"),difference_1)
np.save(os.path.join(link_folder,"08_ep80_difference_ev2.npy"),difference_2)
np.save(os.path.join(link_folder,"08_ep80_difference_ev3.npy"),difference_3)
#%% confusion matrix
...
np.save(os.path.join(link_folder,"08_ep31_hist_miss_ev1.npy"),y_diff_1_map)
np.save(os.path.join(link_folder,"08_ep31_hist_miss_ev2.npy"),y_diff_2_map)
np.save(os.path.join(link_folder,"08_ep31_hist_miss_ev3.npy"),y_diff_3_map)
#%% 2D histogram
...
histofhist(yhat1, y_sim1, ax, ax_histx, ax_histy)
...
# axs[0, 0].hist2d(yhat1, y_sim1, bins=100, cmin = 0.05)
# axs[0, 1].hist2d(yhat2, y_sim2, bins =100, cmin = 0.05)
# axs[0, 2].hist2d(yhat3, y_sim3, bins=100, cmin = 0.05)
